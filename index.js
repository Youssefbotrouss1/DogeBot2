...
console.log(color('  Xeon Bot Inc. 2021','mediumseagreen'))
	...
	dogexeon.on('credentials-updated', () => {
		fs.writeFileSync('./.session.data.json', JSON.stringify(dogexeon.base64EncodedAuthInfo(), null, '\t'))
		info('2', 'Doge Bot Loading...')
	})
	fs.existsSync('./.session.data.json') && dogexeon.loadAuthInfo('./.session.data.json')
